...
static inline uint32_t	i_bytes(t_int2 pos, t_bmp_file f, int bpl)
{
	uint32_t c;
	uint32_t t;
...
static inline t_bmp		*i_decompress(t_bmp_file f)
{
	...
	if ((out = malloc(sizeof(t_bmp)
		...
		return (ft_clear("pp", f.color_palette, f.data));
	out->size = (t_int2){.x = f.dib.width, .y = f.dib.height};
	p.y = -1;
...
t_bmp					*bmp_file_load(const char *path)
{
	...
	if (read(fd, &f.head, sizeof(f.head)) < (int)sizeof(f.head) ||
		...
		|| ft_strchr((char[8]){OS22XBITMAPHEADER, OS22XBITMAPHEADERV,
	...
	ft_bzero(&((char*)&f.dib)[f.dib.dib_size], sizeof(f.dib) - f.dib.dib_size);
